...
class Data {
  ...
  endRecall() {
    ...
    if (this.scorer) {
      let targets = Boolean(this.currentDataBlock.target_list)
        ? this.currentDataBlock.target_list
      ...
      this.scorer.score(currentDataBlock.response, targets).then((score) => {
        ...
        if (Array.isArray(currentDataBlock.score)) {
          ...
          currentDataBlock.cumulative_score =
            prev_score + currentDataBlock.score;
        }
  ...
  setBlankResponseTimesToNull() {
    for (let key in this) {
      if (
        this[key] === -1 &&
        (key === "rt" ||
          key === "rt_first_keypress" ||
          key === "rt_last_keypress")
      ) {
        delete this[key];
      }
    }
